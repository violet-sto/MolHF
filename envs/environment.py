...
def check_chemical_validity(mol):
    ...
    """
    s = Chem.MolToSmiles(mol, isomericSmiles=True)  # must feasible
...
def check_chemical_validity_with_seg(mol, largest_connected_comp=True):
    if mol is None:
        return None
    s = Chem.MolToSmiles(mol, isomericSmiles=True)
    m = Chem.MolFromSmiles(s)
    if largest_connected_comp and '.' in s:
        # 'C.CC.CCc1ccc(N)cc1CCC=O'.split('.')
        vsm = [(c, len(c)) for c in s.split('.')]
...
def steric_strain_filter(mol, cutoff=0.82, max_attempts_embed=20, max_num_iters=200):
    ...
    try:
        ...
        if flag == -1:
            ...
            return False
    # to catch error caused by molecules such as C=[SH]1=C2OC21ON(N)OC(=O)NO
    except:
        # print("Unable to generate 3d conformer")
        return False
    ...
    if AllChem.MMFFHasAllMoleculeParams(m_h):
        ...
        # to deal with molecules such as CNN1NS23(=C4C5=C2C(=C53)N4Cl)S1
        try:
